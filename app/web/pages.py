...
def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    recent_tasks = (
        apply_task_scope(
            db.query(Task).options(joinedload(Task.assignee), joinedload(Task.status), joinedload(Task.priority)),
            user,
        )
...
def tasks_page(
    ...
    return templates.TemplateResponse(
        "tasks.html",
        {
            ...
            "filters": {"status_code": status_code, "priority_code": priority_code, "source_type": source_type},
...
def create_task_from_form(
    ...
    assignee_id: int | None = Form(default=None),
    priority_code: str = Form("medium"),
    status_code: str = Form("new"),
    db: Session = Depends(get_db),
):
    user = get_user_from_cookie(request, db)
    if user is None:
